...
class Telefone {
	...
	private function Valida_Dados(&$MsgErro){ 
	    ...
	    if ($this->NR_Telefone == 0){
		   $MsgErro = 'Numero do Telefone inválido';
		   return FALSE;
		}
		
		if ($this->TP_Mobilidade == null){
			$MsgErro = 'Mobilidade invalida';
			return FALSE;
		}
		
		if ($this->TP_Uso == null){
			$MsgErro = 'Uso inválido';
			return FALSE;
		}
		
		if ($this->CD_DDD == 0){
			$MsgErro = 'DDD inválido';
			return FALSE;
		}
		
		if ($this->SQ_Operadora == 0){
			$MsgErro = 'Operadora invalida';
			return FALSE;
		}
	...
	*/
	private function Existe_Registro(&$MsgErro){
		...
		if (!$result){
			$MsgErro = 'Erro bd ao acessar Telefone: ' . $query . mysql_error();
			return FALSE;
		}
	...
	public function Insert(&$MsgErro){
		...
		if (!($result && (mysql_affected_rows() > 0))) {
			$MsgErro = 'Não foi possivel incluir o telefone: ' . mysql_error() . $query;
			return FALSE;
		}
...
public function Delete(&$MsgErro){
	...
	if (!($result && (mysql_affected_rows() > 0)))
	{
		$MsgErro = 'Não foi possivel excluir o telefone: ' . mysql_error();
		return FALSE;
	}
...
public function GetReg(&$MsgErro){
	...
	if (mysql_num_rows($this->Regs) == 0){
		$MsgErro = 'Telefone não encontrado';
		return FALSE;
	}
	...
	public function Edit(&$MsgErro){
		...
		if (!$result || mysql_affected_rows() == 0){
			$MsgErro = 'Telefone não alterado: ' . mysql_error();
			return FALSE;
		}
		return TRUE;
	}
