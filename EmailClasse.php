...
class Email {
	...
	private function Valida_Dados($MsgErro){ 
	    ...
	    if ($this->TP_Email == null){
		   $this->MsgErro = 'Tipo de Email inválido';
		   return FALSE;
		}
	...
	*/
	...
	public function Insert($MsgErro){
		...
		if (!($result && (mysql_affected_rows() > 0))) {
			$this->MsgErro = 'Não foi possivel incluir o Email: ' . mysql_error();
			return FALSE;
		}
...
public function Delete($MsgErro){
	...
	if (!($result && (mysql_affected_rows() > 0)))
	{
		$this->MsgErro = 'Não foi possivel excluir o Email: ' . mysql_error();
		return FALSE;
	}
...
public function GetReg($MsgErro){
	...
	if (mysql_num_rows($this->Regs) == 0){
		$this->MsgErro = 'Email não encontrado';
		return FALSE;
	}
	...
	public function Edit($MsgErro){
		...
		if (!$result || mysql_affected_rows() == 0){
			$this->MsgErro = 'Email não alterado: ' . mysql_error();
			return FALSE;
		}
		return TRUE;
	}
